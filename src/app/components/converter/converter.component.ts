...
export class ConverterComponent implements OnDestroy {
  ...
  constructor(private rateService: ExchangeRatesService, private fb: FormBuilder){
    rateService.getExchangeRates()
    .pipe(takeUntil(this.unsubscribe$))
    .subscribe(rates => {
      this.rates = rates.concat({ cc: 'UAH', rate: 1 } as Currency).sort((a, b) => a.cc.localeCompare(b.cc));
      this.initForm();
    });
  }
